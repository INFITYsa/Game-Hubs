...
local Window = Fluent:CreateWindow({
    Title = "Mirage Shop Bypass v3.2",
    SubTitle = "by EthosHacker",
    TabWidth = 160,
    Size = UDim2.fromOffset(400, 200),
})

-- Hook principal no sistema de ilhas
local originalInvoke
originalInvoke = hookmetamethod(game, "__namecall", function(self, ...)
    if not checkcaller() and getnamecallmethod() == "InvokeServer" and self.Name == "MirageIsland" then
        local args = {...}
        -- Spoof de localização
        args[2] = Vector3.new(-1250, 40, -7500) -- Coordenadas fixas da Mirage
...
Window:AddButton({
    Title = "Abrir Loja da Mirage",
    Description = "Ativa mesmo sem estar na ilha",
    Callback = function()
        local success = pcall(function()
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("MirageIsland")
        end)
        
        Fluent:Notify({
            Title = success and "Sucesso!" or "Erro",
            Content = success and "Loja aberta" or "Sistema atualizado",
            Duration = 5
        })
    end
})
...
for i = 1, math.random(5,10) do
    table.insert(junkCode, string.char(math.random(65,90))..tostring(math.random()))
end

getgenv().SecureCall = function(f) return f end
